...
tidy_ons_smoking <- function(){
  ...
  # Get the data using the function get_ons_smoking()

  data <- get_ons_smoking()
  ...
  clean_data <- data %>%
    ...
    tidyr::pivot_longer(-all_persons_aged_16_and_over,
                        ...
                        names_to = "names") %>%
    tidyr::fill(values) %>%
    dplyr::mutate(all_persons_aged_16_and_over =
                    dplyr::case_when(is.na(all_persons_aged_16_and_over) & clean_data$values %in% cnames_age ~ "cname_age",
                                     is.na(all_persons_aged_16_and_over) & clean_data$values %in% cnames_gender ~ "cname_gender",
  ...
  cnames <- clean_data %>%
    ...
    cbind(clean_data %>%
            ...
            dplyr::rename(age_values = values,
                          age_names = names)) %>%
    dplyr::select(names,
                  values,
                  age_values) %>%
    tidyr::unite(values, values:age_values, sep = " ") %>%
    dplyr::mutate(all_persons_aged_16_and_over = "header",
                  values =
                    dplyr::case_when(str_detect(one_colname$values, "All persons") == TRUE
                                     ~ stringr::str_replace(one_colname$values, "All persons",
                                                            "Allpersons"),
                                     # Remove space for later use of separate()
                                     # which uses space to determine first part of string
                                     TRUE ~ col_names$values))
  ...
  one_colname <- clean_data %>%
    ...
    dplyr::filter(stringr::str_length(one_colname$header) <= 8,
                  one_colname$header != "Notes") %>%
    dplyr::mutate(header = substring(one_colname$header, 1, 4),
